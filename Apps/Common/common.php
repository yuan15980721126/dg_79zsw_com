...
{
    if (!is_array($array)) return htmlspecialchars(strip_tags($array), ENT_QUOTES);
...
{
    preg_match_all('/u{1}[a-t|v-z|A-T|V-Z|\d]{4}/', $string, $match);
    $buff = $string;
    $match = array_unique($match);
    foreach ($match[0] as $v) {
        $buff = str_replace($v, json_decode('["\\' . $v . '"]')[0], $buff);
    }
