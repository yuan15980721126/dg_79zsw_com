...
class CategoryAction extends AdminbaseAction
{
    ...
    public function insert()
    {
		...
		*/
		...
		if($_POST['catdir']){
			$catdir=$_POST['catdir'];
			$catdirdata=M('Category')->where(" catdir = '$catdir' ")->find();
			if($catdirdata){
				$this->error(L('add_error'),'/index.php?g=Admin&m=Category&a=add');
			}else{
	...
	public function delete() {
		...
		if($this->categorys[$catid]['type']==1){
			...
			if($this->categorys[$catid]['module']=='Page'){
				$module=M('Page');
				$module->delete($catid);
				$where['id'] = array("in" , $arrchildid) ;
			}else{
				$where['catid'] = array("in" , $arrchildid) ;
			}
		...
		public function catdir() {
			$catdir=$_REQUEST['catdir'];
			$data=M('Category')->where(" catdir = '$catdir' ")->find();
			if($data){
				echo 'false';
			}else{
				echo 'true';
