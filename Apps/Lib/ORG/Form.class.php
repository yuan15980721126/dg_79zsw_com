...
class Form extends Think {
    ...
    // add by jzp 副栏目类型的字段的内容显示代码。
    public function catalog($info,$value){
        $validate = getvalidate($info);		
		if(APP_LANG){ 
			$Category = F('Category_'.$this->lang);			
		}else{
		...
		$str  = "<option value='\$id' \$selected>\$spacer \$catname</option>";
		$tree = new Tree ($array);
		$parseStr .= '<select multiple="multiple" style="height:180px;" id="'.$id.'" name="'.$field.'[]" class=" '.$info['class'].'"  '.$validate.'>';
		//$parseStr .= '<option value="">'.L('please_chose').'</option>';
		$parseStr .= $tree->get_tree(0, $str, $value);
		$parseStr .= '</select>';
		return $parseStr;
	...
	public function images($info,$value){
		...
		<div class="c"></div>
		<input type="button" class="button" value="'.L('upload_images').'" onclick="javascript:swfupload(\''.$field.'_uploadfile\',\''.$field.'\',\''.L('uploadfiles').'\','.$this->isadmin.','.$info['setup']['more'].',0,'.$info['setup']['upload_maxnum'].',\''.$info['setup']['upload_allowext'].'\','.$info['setup']['upload_maxsize'].','.$info['moduleid'].',\''.$yourphp_auth.'\',up_images,nodo)">  '.$info['setup']['cc'];
