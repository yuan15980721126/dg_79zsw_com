...
<table  class="search_table" width="100%" <?php if($isChildModule) : ?>style='display:none;'<?php endif;?>>
	<tr>
		<td class="search">
			...
			<?php if($fields['industryx']): ?><option value="industryx" <?php if(($searchtype) == "industryx"): ?>selected="selected"<?php endif; ?>><?php echo L('主营行业');?></option><?php endif; ?>
			...
			<?php if('Kecheng' == $module_name) : ?>
				<select id="school_id" name="school_id">
					<option value="">选择学校</option>
					<?php $schools = M('School')->field('id,title')->select(); $selected = ''; foreach ($schools as $school) { $selected = $_REQUEST['school_id']==$school['id'] ? 'selected' : ''; echo '<option value="', $school['id'], '" ', $selected, '>', $school['title'], '</option>'; } ?>
				</select>
			<?php endif;?>
...
<div class="table-list">
    <table width="100%" cellspacing="0">
		<thead>
			...
			<?php if('Tdk' == $module_name) : ?>
				<th><?php echo L('类型');?></th>
				<th><?php echo L('上级栏目');?></th>
			<?php endif;?>
			<?php if('Tdk'!== $module_name) : ?>
				<th><?php echo L('title');?></th>
			<?php endif;?>
			<?php if('Blacklist'== $module_name) : ?>
				<th>IP</th>
			<?php endif;?>
      	...
      	<tbody>
			...
			<?php if('Tdk' == $module_name) : ?>
				...
				<td align="center">
					...
					<?php else :?>
					<?php
 if($vo['type']==1){ $schoolid = M('school')->where('id='.$vo['child_id'])->find(); echo $schoolid['username']; }elseif($vo['type'] == 2){ $keceng = M('user')->where('schoolid='.$vo['child_id'])->find(); echo $keceng['username']; }elseif($vo['type'] == 3){ $article = M('kecheng')->where('id='.$vo['child_id'])->find(); echo $article['title']; } ?>
					<?php endif;?>
				</td>
			<?php endif;?>
			...
			<?php if('Tdk'!== $module_name) : ?>
				<td>
					...
					<?php if($module_name=='Guestbook' || $module_name=='Feedback') : echo ($vo['username']); ?>
					-<?php echo ($vo['title']); ?>
					<?php else :?>
					<a href="<?php echo ($vo["url"]); ?>" <?php if($vo['title_style']!=''): ?>style ="<?php echo ($vo['title_style']); ?>"<?php endif; ?> target="_blank"><?php echo ($vo['title']); ?>
					</a>
					<?php endif;?>
			<?php endif;?>
					<?php if('Blacklist'== $module_name) : ?>
